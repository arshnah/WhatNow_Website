        ...
        <Section number="01" title="How the pay actually works">
          ...
          <div className="bg-orange-50 border-l-4 border-orange-500 p-6 rounded-r-xl">
            <p className="text-slate-800 leading-relaxed font-medium">
              <strong className="font-black text-orange-700">&quot;Tax-free&quot; — with an asterisk.</strong> A seafarer&apos;s foreign earnings are tax-free in India <em>only</em> if you qualify as a Non-Resident (broadly, by spending enough days outside India in the financial year under the rules for seafarers). Fall short of the day-count and it becomes taxable. This is general information, not tax advice — check the current Income-Tax position for your year.
            </p>
          </div>
        </Section>
          ...
          <div className="overflow-x-auto border border-slate-200 rounded-xl">
            <table className="w-full text-left border-collapse min-w-[700px] bg-white">
              <thead className="bg-slate-50">
                <tr>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Level</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Deck</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Engine</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Typical /month (USD)</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">≈ /month (INR)</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Unlocked by</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={level}>
                    <td className={`p-4 font-black text-slate-700 ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{level}</td>
                    <td className={`p-4 text-slate-600 font-medium ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{deck}</td>
                    <td className={`p-4 text-slate-600 font-medium ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{engine}</td>
                    <td className={`p-4 font-black text-orange-600 ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{usd}</td>
                    <td className={`p-4 text-slate-600 font-medium ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{inr}</td>
                    <td className={`p-4 text-slate-600 font-medium text-sm ${i < arr.length - 1 ? "border-b border-slate-100" : ""}`}>{unlocked}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Section number="03" title="The certification ladder (this gates every promotion)">
          ...
          <div className="overflow-x-auto border border-slate-200 rounded-xl mb-8">
            <table className="w-full text-left border-collapse min-w-[600px] bg-white">
              <thead className="bg-slate-50">
                <tr>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Engine side</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Deck side</th>
                  <th className="p-4 font-black text-xs uppercase tracking-widest text-slate-500 border-b border-slate-200">Roughly what it takes</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="p-4 font-black text-slate-700 border-b border-slate-100"><strong>MEO Class IV</strong> — Fourth Engineer</td>
                  <td className="p-4 font-black text-slate-700 border-b border-slate-100"><strong>2nd Mate CoC</strong> — Third Officer</td>
                  <td className="p-4 text-slate-600 font-medium border-b border-slate-100">~6 months engine-room watchkeeping (ships ≥750 kW) / structured deck sea time, + exams & courses. GME/B.Tech grads are exempt from Part A of MEO IV.</td>
                </tr>
                <tr>
                  <td className="p-4 font-black text-slate-700 border-b border-slate-100"><strong>MEO Class II</strong> — Second Engineer</td>
                  <td className="p-4 font-black text-slate-700 border-b border-slate-100"><strong>Mate (Chief Mate) CoC</strong> — Chief Officer</td>
                  <td className="p-4 text-slate-600 font-medium border-b border-slate-100">~12 months as an engineer officer after Class IV (with enough propelling time on ships ≥750 kW); plus advanced courses, written & oral.</td>
                </tr>
                <tr>
                  <td className="p-4 font-black text-slate-700"><strong>MEO Class I</strong> — Chief Engineer</td>
                  <td className="p-4 font-black text-slate-700"><strong>Master CoC</strong> — Captain</td>
                  <td className="p-4 text-slate-600 font-medium">~12 months as Second Engineer after Class II (ships ≥3000 kW), a ~2-month prep course, plus written & oral — heavy on ship management and maritime law.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <Section number="04" title="A realistic timeline to Fourth Engineer (the honest version)">
          ...
          <ol className="space-y-4 mb-8">
            {[
              ...
              <li key={step} className="flex items-start gap-4 text-slate-700 font-medium">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 mb-4">
        ...
        <Section number="06" title="Life after sailing (the real safety net)">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={role} className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl p-4">
                <span className="text-orange-500 font-black flex-shrink-0">›</span>
                <span className="text-slate-700 font-semibold text-sm">{role}</span>
              </div>
            ))}
          </div>
          <p className="text-slate-600 font-medium mt-6 leading-relaxed">
